...
#	model = [[category2,user_threshold]]
#	print model
	for item in model:
#	   print "item[0]: ",item[0]
#	   print "item[1]: ",item[1]
...
#				print "TP cat:", category2
#				print "TP count so far", len(tphitHistory[category2])
		else: ## FN
...
#	check_novel = [[category1,user_threshold]]
...
#		hitUniq(line)
	    if float(value) > float(item[1]) and part1 not in fphitHistory and part1 not in tphitHistory and part1 not in fnhitHistory:  ## TN Test Added by Fahim on 20110824. Diff category and above thres i.e. true rejection
...
#	res.append( str(get_thresh(cat)) + ","+ cat + "," + str(u1categoryCount[cat]) + "," + str(u2categoryCount[cat]) + "," + str(int(categoryCount[cat])) + "," + str(int(u1categoryCount[cat] + u2categoryCount[cat])) + "," + str(int(u1categoryCount[cat] * u2categoryCount[cat]) ) + "," + str(categoryMatches[cat]) + "," +  str(categoryFP[cat]) +  "," + str(categoryFN[cat]) + "," + str(categoryTN[cat]) + "," + str(tprcat) + "," + str(fprcat) + "," + str(acccat) + "," + str(auccat) + "\n" )
    ...
    tpr_final = str(float( sum(tphit_list)/(sum(tphit_list)+sum(fnhit_list))) ) 
    fpr_final = str(float( sum(fphit_list)/(sum(fphit_list)+(tnsum))) ) 
    p = float(sum(tphit_list)+sum(fnhit_list))
    tpr_f = sum(tphit_list)/p
    auc_final = (1+tpr_f-float(fpr_final) )/2.0

    out.write(","+str(sum(dataset_list))+","+str(sum(countsig_list))+","+str(sum(tphit_list))+","+str(sum(fphit_list))+","+str(sum(fnhit_list))+","+str(tnsum)+","+ str(sum(tphit_list)/p)  + "," + str(float( sum(fphit_list)/(sum(fphit_list) +(tnsum))) ) +  " ,"+str(xhitsum)+","+str(t_accuracy)+","+str(p))
    out.write("\nOutliers:\n")
    outlier = outlier - classified.intersection(outlier)
...
#    for item in temp_outlier:
    ...
    out.close()
    ## NOW WRITE ROC RESULTS TO A SEPARATE SUMMARY FILE
    myresult = open("roc-results.txt","a")
    myresult.write(str(user_threshold) + "," +str(sum(dataset_list))+","+str(sum(countsig_list))+","+str(sum(tphit_list))+","+str(sum(fphit_list))+","+str(sum(fnhit_list))+","+str(tnsum)+","+ str(sum(tphit_list)/p)  + "," + str(fpr_final) +  " ,"+str(xhitsum)+","+str(t_accuracy)+"," + str(auc_final)  +"," + str(p) + "," + str(sum(tphit_list)/p)  +"\n")
...
def loadThresh(filename):
	...
	for line in f:
		...
		threshDict[cat] = t
#===========================================================
# Load random exemplars for each class in a dict loadExemplars(filename)
#===========================================================
def loadExemplars(filename):
#	global threshDict;
	global user_threshold
	f = open(filename,'r')
	m = open("mymodel.txt",'w')
	for line in f:
		if line == "": continue  ## ignore empty lines
		line = line.strip('\n')
		exemplar = line.strip(',')
		m.write(exemplar + "," + user_threshold + "\n" )
	m.close()
#		threshDict[cat] = []
#		threshDict[cat] = t
...
def roc(xlist,ylist,filename,title,xlabel,ylabel):
	...
	## Save plot as PNG
	pylab.grid(True)
	pylab.savefig(filename + "-" + user_threshold  + '.png')
	pylab.close()
...
#	infile = sys.argv[1]	## input result file to process
	...
	if len(sys.argv) < 3:
		print "Usage: \npython get_roc_for_random.py user_threshold metric outputfolder"
		sys.exit
	user_threshold = sys.argv[1] ## user defined threhold
	metric = sys.argv[2]	## Metric used to determine file
	resultf = sys.argv[3] ## output folder
	exemp = sys.argv[4] ## number of exemplars used

	#threshfile = sys.argv[3] ## path to threshold file
	#========================================================================
#	loadThresh("perclassthresh.txt")	## load the threshold file and build a dictionary
	loadExemplars("random_exemplars.txt") ## load random exemplars, add fixed threshold and write to mymodel.txt
	...
	output = "%s-500-dyn-result-%s"%(int(time.time()),exemp)
	output = output + "-composite-" + metric + "-" + user_threshold + ".csv"
	print topLevelCategories
...
##### NO NEED TO CREATE PER CLASS ROC #####
